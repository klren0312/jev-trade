...
2. **40% 的部分减仓按几何级数衰减，永远到不了 1e-12 的清零线**：残值一直算作持仓，既占 `maxPositions` 名额，又让建仓扫描 `if (held) continue` 永久跳过这个币。现在卖出时若剩余名义额低于权益的 1%（`JEV_DUST_PCT`）直接清仓，`positions/cost/peak/entryAt` 一起删掉，该币重新可买；建仓判断也改为按名义额而不是 `qty > 0`。
