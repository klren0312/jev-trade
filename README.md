...
**为什么不把买入交给模型**：实测线上 Jev 极保守——334 次动量问答里「这波会不会延续」的中位数只有 **0.34**（p05 0.22、p95 0.40、最大 0.47，**从未**达到 0.6），新闻材料性最高 0.42。结果是 194 次决策 **0 次买入**，账面长期空仓。这类绝对阈值闸门在模型侧不可达，所以建仓改由确定性规则直接执行（`source: "entry"`，日志里标明「未询问 Jev」），模型仍然负责平仓、止盈和复核。

## 平仓不能把仓位磨成碎仓（`src/exchange.mjs`）

第一条规则上线后出现新问题：**3 笔买入对 70 笔卖出**，两个观察币各自只剩 0.00003 / 0.01 个币的残值，此后再没有买过。两个原因：

1. **动量的「反转」腿照抄了对称阈值**（`cont ≤ 0.4` 就反向操作），而模型答案本来就分布在 0.4 以下 → 96% 的上涨扫描都触发减仓。现在两条腿各自标定：续势 `cont ≥ 0.6`（实测几乎不发生）、反转 `cont ≤ 0.22`（观测分布的最左尾），中间一律 skip。
2. **40% 的部分减仓按几何级数衰减，永远到不了 1e-12 的清零线**：残值一直算作持仓，既占 `maxPositions` 名额，又让建仓扫描 `if (held) continue` 永久跳过这个币。现在卖出时若剩余名义额低于权益的 1%（`JEV_DUST_PCT`）直接清仓，`positions/cost/peak/entryAt` 一起删掉，该币重新可买；建仓判断也改为按名义额而不是 `qty > 0`。买回一个只剩碎仓的币时，会先把碎仓按现价清算掉——否则新仓位会继承旧仓的 `peak`，移动止盈对着一个其实刚开的仓位直接触发。
...
重启后的头一个小时里 30/60 分钟涨跌窗口本来是空的——`chgPct()` 过去会退化成「相对开机时刻的变化」，把回调建仓腿看成一条横盘，所以每次部署后建仓扫描都会先闲着。现在 RSI 用的那批 1 分钟 K 线会顺便回填这条历史，覆盖不到的窗口返回 null 而不是给出一个看起来合理的错数字。
...
| `JEV_CONT_MIN` `JEV_FADE_MAX` | `0.6` / `0.22` | 动量两条腿的闸门：续势买/卖、反转卖/买（中间不动） |
| `JEV_DUST_PCT` | `0.01` | 部分减仓后剩余名义额低于权益的 1% 直接清仓，释放该币的建仓名额 |
